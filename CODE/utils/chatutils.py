...
class FailedToRetrieveRunErr(Exception):
    def __init__(self, message="Failed to retrieve run."):
...
def retry(max_attempts=3, delay=1, exceptions=(Exception,)):
    """
    Decorator for retrying a function multiple times with a delay between attempts.
    
    Args:
        max_attempts (int): Maximum number of retry attempts.
        delay (float): Delay in seconds between retry attempts.
        exceptions (Tuple[Exception]): Tuple of exceptions to catch and retry upon.
    """
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
...
class AssistantSession(ChatSession):
    ...
    def __init__(self,gpt_name='Assistant',model="",instructions:str="",tools:list[dict[str,str]]=[],file_paths:list[str]=[""],api_key:str="") -> None:
        ...
        self.client = openai.OpenAI(api_key=api_key) #, default_headers={"OpenAI-Beta": "assistants=v1"
        ...
        assert self.assistant is not None, "Failed to create assistant." + print(e)
    ...
    def __get_input(self,user_input=None):
        if user_input is None:
            user_input = input("> ")
        return user_input
    ...
    def __get_reply(self,user_input,*args,**kwargs):
        ...
        run = self.__create_run(user_message)

        max_tries = 20 
        while not run.completed_at and max_tries>0:
            time.sleep(1)
            run = self.client.beta.threads.runs.retrieve(thread_id=self.thread.id, run_id=run.id)
            if run.failed_at: 
                print(run.last_error)
                raise FailedToRetrieveRunErr
            max_tries-=1

        if max_tries==0: raise FailedToRetrieveRunErr

        while len(self.messages)<_length_before+2: time.sleep(1)
    ...
    def __make_assistant(self):
        ...
        self.assistant = self.client.beta.assistants.create(
            ...
            tools=self.tools,
            model=self.model,
            file_ids=[file.id for file in self.files] # v1
            # v2
            # tool_resources={
            #     "file_search": {
            #         'vector_store_ids': [file.id for file in self.files]}
                # "code_interpreter": {
    ...
    def _unpack_message(self,msg):
        try: msg.content[0]
        except: raise FailedToRetrieveRunErr
