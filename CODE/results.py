...
def anonymised_graphic():
    ...
    # PLOT
    # Define hex colors
    colors = ['#F8F8F8', '#DAEEF3', '#BAE0EA', '#77C5D8', '#449DC2', '#206A8A']
    ...
    growth_data = growth_data.rank(axis=1, method='max', ascending=False)
    ...
    carbon_data = carbon_data.rank(axis=1, method='max', ascending=False)
    ...
    # replace index with Company A, Company B, etc. 
    # growth_data.index = [f'Company {i}' for i in range(1, len(growth_data) + 1)]
    # carbon_data.index = [f'Company {i}' for i in range(1, len(carbon_data) + 1)]
    ...
    heatmap_growth = axs[0].imshow(growth_data, cmap=growth_cm, vmin=growth_data.T.min().min() - 0.5, vmax=growth_data.T.max().max() + 0.5)
    cbar_growth = fig.colorbar(heatmap_growth, ax=axs[0], ticks=range(growth_N_colors), label='Score')
    cbar_growth.set_ticklabels([0] + list(range(growth_temp_min, growth_temp_max + 1)))

    axs[0].set_xticks(np.arange(len(growth_data.columns))-0.5)
    # axs[0].grid()
    axs[0].set_xticks(range(len(growth_data.columns)))
    ...
    heatmap_carbon = axs[1].imshow(carbon_data, cmap=carbon_cm, vmin=carbon_data.T.min().min() - 0.5, vmax=carbon_data.T.max().max() + 0.5)
    cbar_carbon = fig.colorbar(heatmap_carbon, ax=axs[1], ticks=range(carbon_N_colors), label='Score')
    cbar_carbon.set_ticklabels([0] + list(range(carbon_temp_min, carbon_temp_max ))) # + 1
